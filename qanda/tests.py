...
def create_answer(question, answer_text, votes):
    """
...
class QuestionDetailViewTests(TestCase):
    ...
    def test_answer_created(self):
        ...
        """
        some_question = create_question(question_text='Some Question.', days=-5)
        answer_one = create_answer(question=some_question, answer_text="answer one", votes=0)
        answer_two = create_answer(question=some_question, answer_text="answer two", votes=5)
...
class QuestionResultsViewTests(TestCase):   
    ...
    def test_answers_and_votes(self):
        """
        The results view should display a question, it's answers along with the 
        votes for each answer
        """
        some_question = create_question(question_text='Some Question.', days=-5)
        answer_one = create_answer(question=some_question, answer_text="answer one", votes=0)
        answer_two = create_answer(question=some_question, answer_text="answer two", votes=5)
        url = reverse('qanda:results', args=(some_question.id,))
        response = self.client.get(url)
        self.assertContains(response, some_question.question_text)
        self.assertContains(response, answer_one.answer_text)
        self.assertContains(response, answer_two.answer_text)
        self.assertContains(response, answer_one.votes)
        self.assertContains(response, answer_two.votes)
